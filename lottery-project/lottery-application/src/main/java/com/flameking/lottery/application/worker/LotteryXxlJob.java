        ...
        while (!activityVOList.isEmpty()) {
            for (ActivityVO activityVO : activityVOList) {
                ...
                switch (state) {
                    ...
                    case 4:
                        stateHandler.setActivityState(ActivityStateSupport.getActivityState(Constants.ActivityState.PASS));
                        Result state4Result = stateHandler.doDoing(activityVO.getActivityId());
                    ...
                    case 5:
                        stateHandler.setActivityState(ActivityStateSupport.getActivityState(Constants.ActivityState.DOING));
                        if (activityVO.getEndDateTime().before(new Date())){
                            Result state5Result = stateHandler.doClose(activityVO.getActivityId());
